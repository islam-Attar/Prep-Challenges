...
function fullName(arr) {
    const names = arr.map(x => `${x.firstName} ${x.lastName}`)
    return names
}
...
function studentsResult(arr) {
    const newArr = arr.map((element => ({ 
        ...element,
        ...( ((element.gradsList.reduce((x, y) => x + y)) / element.gradsList.length ) >= 50 && { result: "Passed" }),
        ...( ((element.gradsList.reduce((x, y) => x + y)) / element.gradsList.length ) < 50 && { result: "Failed" }),
