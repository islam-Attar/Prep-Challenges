...
const objectCounter = (obj) => {
    const count = obj.reduce((counter, x) => x.voter_Name != null ? counter += 1 : counter, 0 ); 
    return count ;
...
const stringReverse = (str) => {
    ...
    const reversedString = newArr.reduce((revWord, word) => {
     return word + " " + revWord 
 },)
...
const statistics = (obj) => {
    const output = obj.reduce((A, order) => {
        return { ...A, [order.votes_To]: (A[order.votes_To] || 0) + 1 };
      }, {});
